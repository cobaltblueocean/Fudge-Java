...
public class FudgeDate implements DateProvider {
  ...
  public static FudgeDate from(ZonedDateTime expiry) {
    return new FudgeDate(expiry.getYear(), expiry.getMonthOfYear().getValue(), expiry.getDayOfMonth());
  }

  public static FudgeDate from(org.threeten.bp.ZonedDateTime expiry) {
    return new FudgeDate(expiry.getYear(), expiry.getMonthValue(), expiry.getDayOfMonth());
  }

  public static FudgeDate ofYearMonth(int year, int month) {
    return new FudgeDate(year, month);
  }

  public static FudgeDate ofYear(int year) {
    return new FudgeDate(year);
  }
   ...
   * @param instant the date corresponding to this instant at UTC will initialize the object
   */
  protected FudgeDate(final org.threeten.bp.Instant instant) {
    this((DateProvider) ZonedDateTime.ofInstant(Instant.ofEpochNanos(instant.getEpochSecond() * 1000000 + instant.getNano()), TimeZone.UTC));
  }
   ...
   * @param localDate the {@link LocalDate} representation of the date
   */
  protected FudgeDate(final org.threeten.bp.LocalDate localDate) {
   ...
   * Creates a new {@link FudgeDate} object.
   *
   * @param localDate the {@link org.threeten.bp.LocalDateTime} representation of the date
   */
  protected FudgeDate(final org.threeten.bp.LocalDateTime localDate) {
    this(localDate.getYear(), localDate.getMonthValue(), localDate.getDayOfMonth());
  }
   ...
   * @param localDate the {@link org.threeten.bp.LocalDateTime} representation of the date
   */
  protected FudgeDate(final org.threeten.bp.OffsetDateTime localDate) {
    this(localDate.getYear(), localDate.getMonthValue(), localDate.getDayOfMonth());
  }
